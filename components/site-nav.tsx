  ...
  return (
    <header className="sticky top-0 z-50 border-b bg-background/80 backdrop-blur-md">
      ...
      <nav className="mx-auto flex max-w-3xl items-center justify-between px-6 py-3.5">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-2 font-mono font-medium tracking-tight transition-opacity hover:opacity-80"
        >
          <DispoIcon size={16} />
          <span className="text-sm tracking-wider uppercase">dispochat</span>
        </Link>
        ...
        <div className="hidden items-center gap-1 md:flex">
          ...
          {pathname === "/" && (
            <p className="ml-3 animate-[pulse_1.2s_infinite] text-xs text-muted-foreground">
              Press <Kbd>d</Kbd> to toggle light/dark
            </p>
          )}
        </div>
        ...
        <button
          className="flex items-center justify-center gap-1.5 rounded-full border px-3 py-1.5 font-mono text-xs text-muted-foreground transition-colors hover:text-foreground md:hidden"
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
      ...
      <div
        className={`overflow-hidden border-b bg-background/95 backdrop-blur-md transition-all duration-300 ease-in-out md:hidden ${
